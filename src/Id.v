...
Ltac prove_with th :=
  ...
  repeat
    ...
    | H: ?n <= ?m |- {_}               + {Id ?n i<= Id ?m} => right
    | H: ?n <= ?m |- {Id ?n i<= Id ?m} + {_}               => left
    | H: Id ?n i> Id ?m |- _ => inversion_clear H
    | H: Id ?n i< Id ?m |- _ => inversion_clear H
    | H: Id ?n i<= Id ?m |- _ => inversion_clear H
    end;
  try (constructor; assumption); try congruence.
...
Proof. intros. destruct (id_eq_dec x x). reflexivity. contradiction. Qed.

Lemma neq_id : forall (T:Type) x y (p q:T), x <> y -> (if id_eq_dec x y then p else q) = q.
Proof. intros. destruct (id_eq_dec x y). contradiction. reflexivity. Qed.

Lemma lt_gt_id_false : forall id1 id2 : id,
    id1 i> id2 -> id2 i> id1 -> False.
Proof. prove_with le_gt_dec. lia. lia. Qed.
...
Proof. prove_with gt_eq_gt_dec. exfalso. prove_with gt_eq_gt_dec. lia. lia. lia. Qed.
    
Lemma eq_gt_id_false : forall id1 id2 : id,
    id1 = id2 -> id1 i> id2 -> False.
Proof. prove_with gt_eq_gt_dec. injection H. lia. lia. injection H. lia. Qed.
