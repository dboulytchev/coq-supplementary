...
Module SmokeTest.
  ...
  Proof.
    ...
    intro.
    {
      destruct c'.
      destruct p.
      specialize (while_false (Nat 1) (SKIP) s l0 l c H) as Hcond_zero.
      inversion Hcond_zero.
    }
    {
      inversion H;
      subst;
  ...
  Proof.
    ...
    destruct c'.
    destruct p.
    specialize (while_false (Nat 1) s s0 l0 l c H) as Hcond_zero.
    inversion Hcond_zero.
  Qed.
...
Module Renaming.
  ...
  Proof.
      induction s; simpl; 
      try rewrite (Hinv i); try rewrite Renaming.re_rename_expr; try assumption;
      try rewrite IHs1; try rewrite IHs2; try rewrite IHs; try reflexivity.
  Qed.
  ...
  Proof.
    induction st; simpl; destruct r; simpl; unfold update; reflexivity.
  Qed.
